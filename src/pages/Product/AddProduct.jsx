...
const AddProduct = () => {
  ...
  // When the selected category is "Others", inject "Others" as a subcategory
  // option so users can pick it even if no such row exists in the DB.
  const subCategoryOptions = useMemo(() => {
    const list = [...subCategoryData];
    if (
      productData.category?.toLowerCase() === "others" &&
      !list.some((s) => s.subcategory?.toLowerCase() === "others")
    ) {
      list.push({ _id: "__others__", subcategory: "Others" });
    }
    return list;
  }, [subCategoryData, productData.category]);
